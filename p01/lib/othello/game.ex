defmodule Othello.Game do
  ...
  def restart(currentPlayerName) do
    game = currentStatus currentPlayerName
    nxtGame = startGame ""
    nxtGame = %{nxtGame 
                     | players: game.players, speculators: game.speculators, livePlayers: game.livePlayers}
    nxtGame = %{nxtGame 
                     | colors: nxtGame.colors 
                     |> List.update_at(27, fn _ -> 2 end)
                     |> List.update_at(28, fn _ -> 1 end)
                     |> List.update_at(35, fn _ -> 1 end)
                     |> List.update_at(36, fn _ -> 2 end)}
    changeStatus %{"state" => nxtGame}, currentPlayerName  
  end
  ...
  def exitGame(currentPlayerName, playerName) do
    ...
    if game && Enum.member? game.players, playerName do
      ...
      if game.livePlayers > 0 do
        ...
        %{"state" => resp["state"], 
          "msg" => playerName <> " (player) exits", 
          "type" => "warning", "label" => "user:leave"}
      else
        :ok = removeStatus currentPlayerName
        %{"msg" => playerName <> " (last player) exited the game. Cannot play the game anymore", 
          "type" => "warning", "label" => "table:close"}
      end
    else
        ...
        if Enum.member? activeGame.players, playerName do
          ...
          if String.length(playerName) > 0 do
            if Enum.member? activeGame.speculators, playerName do
              ...
              %{"state" => activeGame, 
                "msg" => "Name unknown, watching current game", 
                "type" => "info"}
          end 
        end
      end
  ...
  def nextStatus(currentPlayerName, index) do
    ...
    case changeColors(colors, player, index) do
      {true, colors} -> {true, activeGame 
                                |> Map.put(:colors, colors) 
                                |> Map.put(:turn, 1 - activeGame.turn) 
                                |> isWin
  ...
  def changeStatus(state, currentPlayerName) do
    games = collectGames
    startGames = %{games | currentPlayerName => state}
    :ok = Agent.update(:games, fn last -> startGames end)
    state
  end
